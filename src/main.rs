...
impl Into<piston_window::types::Vec2d> for Vec2 {
  fn into(self) -> piston_window::types::Vec2d {
    [self.x, self.y]
  }
}
...
fn main() -> Result<(), context::SolveError> {
  ...
  let mut window : PistonWindow = WindowSettings::new("Geopad!", [640, 480]).build().unwrap();
