...
impl<T: Clone + Eq + Hash> SpatialHashTable<T> {
  ...
  pub fn insert_line(&mut self, ent: T, l: Line, vp: &Viewport) {
    ...
    if let Some((p1, p2)) = l.to_actual(vp).intersect(aabb) {
      println!("p1: {:?}, p2: {:?}", p1, p2);
      let (init_x_tile, init_y_tile) = self.get_unlimited_cell(p1, vp);
      let (end_x_tile, end_y_tile) = self.get_unlimited_cell(p2, vp);
      if init_x_tile == end_x_tile {
        println!("init_x_tile: {}", init_x_tile);
        ...
        if dir.y < 0.0 {
          ...
          while 0 <= curr_x_tile && curr_x_tile < self.x_tiles as i64 && 0 <= curr_y_tile && curr_y_tile <= self.y_tiles as i64 {
            ...
            for tile_x in curr_x_tile..next_x_tile {
  ...
  fn get_unlimited_cell(&self, p: Point, vp: &Viewport) -> (i64, i64) {
    let Vector2 { x, y } = p;
