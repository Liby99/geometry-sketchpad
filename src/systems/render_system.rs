...
fn draw_line(line: &Line, style: &LineStyle, context: Context, graphics: &mut G2d) {
  let x_min = -480.;
  let x_max  = -x_min;
  let y_min  = -360.;
  let y_max = -y_min;
  ...
  match (p1, p2) {
    (Some(from), Some(to)) => {
      line_from_to(style.color.into(), style.width, [480. + from[0], 360. - from[1]], [480. + to[0], 360. - to[1]], context.transform, graphics);
    },
    _ => ()
  }
}

fn draw_point(point: &Point, style: &PointStyle, context: Context, graphics: &mut G2d) {
  let Point(pos) = point;
  ellipse(
    style.color.into(),
