...
use piston_window::{Button, ButtonArgs, ButtonState, Input, Motion, MouseButton, ResizeArgs};
use specs::prelude::*;
use std::time::SystemTime;

use crate::utilities::piston_key_to_key;
    ...
    match input {
            ...
            match button {
                Button::Mouse(MouseButton::Left) => {
                    ...
                    if is_pressed {
                        ...
                        mouse_event_channel
                            .single_write(MouseEvent::MouseDown(input_state.mouse_abs_pos));
                    } else {
                        mouse_event_channel
                            .single_write(MouseEvent::MouseUp(input_state.mouse_abs_pos));
                        if input_state.is_mouse_left_button_dragging {
                            input_state.is_mouse_left_button_dragging = false;
                            mouse_event_channel
                                .single_write(MouseEvent::DragEnd(input_state.mouse_abs_pos));
                        } else {
                            if let Some(last_pressed) = input_state.mouse_left_button_last_pressed {
                                if last_pressed.elapsed().unwrap().as_millis()
                                    < CLICK_TIME_THRESHOLD
                                {
                                    mouse_event_channel
                                        .single_write(MouseEvent::Click(input_state.mouse_abs_pos));
                                }
                            }
                        }
                    }
                ...
                Button::Keyboard(piston_key) => {
                    let key = piston_key_to_key(piston_key, scancode);
        ...
        Input::Move(motion) => match motion {
            ...
            Motion::MouseRelative(rel_mov) => {
                ...
                if input_state.is_mouse_left_button_dragging {
                    mouse_event_channel.single_write(MouseEvent::DragMove(
                        scrn_rel_mov,
                        input_state.mouse_abs_pos,
                    ));
                } else {
                    if input_state.mouse_left_button.is_activated() {
                        input_state.is_mouse_left_button_dragging = true;
                        mouse_event_channel
                            .single_write(MouseEvent::DragBegin(input_state.mouse_abs_pos));
                    }
                }
            }
            _ => (),
