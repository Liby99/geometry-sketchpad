...
impl ToVirtual for ScreenPosition {
    ...
    fn to_virtual(self, vp: &Viewport) -> Self::Output {
        ...
        let v_x = (s_x - vp.half_screen_width()) / vp.screen_width() * vp.virtual_width()
            + vp.virtual_center.x;
        let v_y = (vp.half_screen_height() - s_y) / vp.screen_height() * vp.virtual_height()
            + vp.virtual_center.y;
...
impl ToScreen for VirtualPosition {
    ...
    fn to_screen(self, vp: &Viewport) -> Self::Output {
        ...
        let s_x = (v_x - vp.virtual_center.x + vp.half_virtual_width()) / vp.virtual_width()
            * vp.screen_width();
        let s_y = (vp.virtual_center.y - v_y + vp.half_virtual_height()) / vp.virtual_height()
            * vp.screen_height();
