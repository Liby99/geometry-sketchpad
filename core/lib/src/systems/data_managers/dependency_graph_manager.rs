            ...
            for event in geometry_event_channel.read(reader) {
                match event {
                    GeometryEvent::Inserted(ent, geom, _) => match geom {
                        Geometry::Point(sym_point, _) => {
                            insert_point(ent, sym_point, &mut *dependency_graph)
                        }
                        Geometry::Line(sym_line, _) => {
                            insert_line(ent, sym_line, &mut *dependency_graph)
                        }
                        Geometry::Circle(sym_circle, _) => {
                            insert_circle(ent, sym_circle, &mut *dependency_graph)
                        }
                    },
                    GeometryEvent::Removed(ent, geom, _) => {
                        ...
                        match geom {
                            Geometry::Point(sym_point, _) => {
                                remove_point(ent, sym_point, &mut *dependency_graph)
                            }
                            Geometry::Line(sym_line, _) => {
                                remove_line(ent, sym_line, &mut *dependency_graph)
                            }
                            Geometry::Circle(sym_circle, _) => {
                                remove_circle(ent, sym_circle, &mut *dependency_graph)
                            }
                        }
                    }
                    _ => (),
...
fn remove_point(ent: &Entity, sym_point: &SymbolicPoint, dependency_graph: &mut DependencyGraph) {
    match sym_point {
        ...
        SymbolicPoint::OnCircle(circle_ent, _) => {
