...
impl<'a> System<'a> for HistoryEventHandler {
    ...
    fn setup(&mut self, world: &mut World) {
        ...
        self.history_event_reader =
            Some(world.fetch_mut::<HistoryEventChannel>().register_reader());
    }

    fn run(
        &mut self,
        (history_event_channel, mut command_event_channel, mut history): Self::SystemData,
    ) {
                ...
                match event {
                        ...
                        if let Some(modification) = history.undo() {
                            match modification {
                                Modification::InsertMany(insertions) => {
                                    write_remove_events(&mut command_event_channel, insertions)
                                }
                                Modification::RemoveMany(removals) => {
                                    write_insert_events(&mut command_event_channel, removals)
                                }
                                ...
                                Modification::HideMany(unhidden_ents) => {
                                    write_unhide_events(&mut command_event_channel, unhidden_ents)
                                }
                                Modification::UnhideMany(hidden_ents) => {
                                    write_hide_events(&mut command_event_channel, hidden_ents)
                                }
                            }
                        }
                    }
                    HistoryEvent::Redo => {
                        if let Some(modification) = history.redo() {
                            match modification {
                                Modification::InsertMany(insertions) => {
                                    write_insert_events(&mut command_event_channel, insertions)
                                }
                                Modification::RemoveMany(removals) => {
                                    write_remove_events(&mut command_event_channel, removals)
                                }
                                Modification::UpdatePoint(ent, old_sym_point, new_sym_point) => {
                                    write_update_event(
                                        &mut command_event_channel,
                                ...
                                Modification::HideMany(unhidden_ents) => {
                                    write_hide_events(&mut command_event_channel, unhidden_ents)
                                }
                                Modification::UnhideMany(hidden_ents) => {
                                    write_unhide_events(&mut command_event_channel, hidden_ents)
                                }
                            }
                        }
                    }
...
fn write_remove_events(
    command_event_channel: &mut CommandEventChannel,
    entities: &HashMap<Entity, Geometry>,
) {
    for (entity, _) in entities {
        command_event_channel
            .single_write(CommandEvent::Remove(RemoveEvent::RemoveByHistory(*entity)));
    }
}

fn write_insert_events(
    command_event_channel: &mut CommandEventChannel,
    entities: &HashMap<Entity, Geometry>,
) {
    for (ent, geometry) in entities {
        let command = match geometry {
            Geometry::Point(sym_point, point_style) => CommandEvent::PointInsert(
                InsertPointEvent::InsertPointByHistory(*ent, *sym_point, *point_style),
            ),
            Geometry::Line(sym_line, line_style) => CommandEvent::LineInsert(
                InsertLineEvent::InsertLineByHistory(*ent, *sym_line, *line_style),
            ),
            Geometry::Circle(sym_circle, circle_style) => CommandEvent::CircleInsert(
...
fn write_update_event(
    ...
    command_event_channel.single_write(CommandEvent::Update(UpdateEvent::UpdatePointByHistory(
        *ent,
        *old_sym_point,
        *new_sym_point,
    )));
