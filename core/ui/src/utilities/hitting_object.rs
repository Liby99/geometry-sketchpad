use core_lib::{components::screen_shapes::*, math::*, resources::*, utilities::*};
use specs::prelude::*;
...
pub fn hitting_object<'a>(
    ...
    let neighbor_entities =
        spatial_entity_map.get_entities_near_point(mouse_pos.into(), threshold.into());
    for entity in neighbor_entities {
        if let Some(p) = scrn_points.get(entity) {
            let dist = (*p - mouse_pos).magnitude();
            if dist < threshold
                && (maybe_selected_point.is_none() || dist < maybe_selected_point.unwrap().1)
            {
            ...
            if dist < threshold
                && (maybe_selected_line.is_none() || dist < maybe_selected_line.unwrap().1)
            {
            ...
            if dist < threshold
                && (maybe_selected_circle.is_none() || dist < maybe_selected_circle.unwrap().1)
            {
    ...
    maybe_selected_point
        .or(maybe_selected_line)
        .or(maybe_selected_circle)
        .map(|(ent, _)| ent)
}
