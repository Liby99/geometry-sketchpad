...
impl<'a> System<'a> for SnapPointRenderer {
  ...
  fn run(
    &mut self,
    (entities, maybe_snap_point, default_point_style, mut scrn_points, mut point_styles): Self::SystemData,
  ) {
    ...
    if let Some(snap_point) = maybe_snap_point.get() {
      ...
      if let Err(err) = scrn_points.insert(ent, snap_point.position) {
        panic!(err)
      }
      if let Err(err) = point_styles.insert(ent, point_style) {
        panic!(err)
      }
    } else {
