...
impl<'a> System<'a> for SnapPointViaMouse {
    ...
    fn run(
        ...
        if tool_state.need_snap_point() {
            ...
            let neighbor_entities = spatial_entity_map
                .get_entities_near_point(mouse_pos.into(), SNAP_TO_POINT_THRES.into());
            ...
            for entity in neighbor_entities {
                if let Some(p) = scrn_points.get(entity) {
                    ...
                    if norm_dist < 1.0 {
                        if maybe_smallest_dist_to_point.is_none()
                            || norm_dist < maybe_smallest_dist_to_point.unwrap()
                        {
                    ...
                    if norm_dist < 1.0 && !is_snapping_to_point {
                        ...
                        if maybe_smallest_dist_to_line.is_none()
                            || norm_dist < maybe_smallest_dist_to_line.unwrap()
                        {
                    ...
                    if norm_dist < 1.0 && !is_snapping_to_point {
                        ...
                        if maybe_smallest_dist_to_circle.is_none()
                            || norm_dist < maybe_smallest_dist_to_circle.unwrap()
                        {
            ...
            if let Some(snap_point) = maybe_snap_point_on_point
                .or(maybe_snap_point_on_line)
                .or(maybe_snap_point_on_circle)
            {
            ...
            if !is_snapping_to_point {
                ...
                for comb in closest_lines.iter().combinations(2) {
                    if let &[(l1_ent, l1), (l2_ent, l2)] = &*comb {
                        ...
                        if let Some(itsct) = l1.intersect(l2) {
                            let norm_dist =
                                (mouse_pos - itsct).magnitude() / SNAP_TO_INTERSECTION_THRES;
                            if norm_dist < 1.0 {
                                if maybe_smallest_dist.is_none()
                                    || norm_dist < maybe_smallest_dist.unwrap()
                                {
                                    ...
                                    maybe_snap_point.set(SnapPoint {
                                        position: itsct,
                                        symbol: SnapPointType::SnapOnLineLineIntersection(
                                            *l1_ent, *l2_ent,
                                        ),
                                    });
                ...
                if !has_line_line_itsct {
                    ...
                    for ((line_ent, line), (circle_ent, circle)) in
                        closest_lines.iter().cartesian_product(&closest_circles)
                    {
                        let ci = line.intersect(*circle);
                        check_circle_intersection(
                            mouse_pos,
                            ci,
                            maybe_smallest_dist.clone(),
                            &mut |m| match m {
                                Some((p, norm_dist, ty)) => {
                                    maybe_smallest_dist = Some(norm_dist);
                                    maybe_snap_point.set(SnapPoint {
                                        position: p,
                                        symbol: SnapPointType::SnapOnCircleLineIntersection(
                                            *circle_ent,
                                            *line_ent,
                                            ty,
                                        ),
                                    ...
                                    &mut |m| match m {
                                        Some((p, norm_dist, ty)) => {
                                            ...
                                            maybe_snap_point.set(SnapPoint {
                                                position: p,
                                                symbol:
                                                    SnapPointType::SnapOnCircleCircleIntersection(
                                                        *c1_ent, *c2_ent, ty,
                                                    ),
                                            });
                                        }
...
fn check_circle_intersection<F>(
    ...
    F: FnMut(Option<(ScreenPosition, f64, CircleIntersectId)>) -> (),
{
    match ci {
