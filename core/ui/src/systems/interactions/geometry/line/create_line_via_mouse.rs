...
use core_lib::{components::symbolics::*, events::*, math::*};
use specs::prelude::*;
...
impl<'a> System<'a> for CreateLineViaMouse {
    ...
    fn setup(&mut self, world: &mut World) {
        ...
        self.tool_change_event_reader = Some(
            world
                ...
                match tool {
                    Tool::Line(_) => {
                        if self.active_point_event_reader.is_none() {
                            self.active_point_event_reader =
                                Some(active_point_event_reader.register_reader())
                        }
                    }
                    _ => {
                    ...
                    if first_point_ent != curr_ent {
                        if let Tool::Line(line_type) = tool_state.get() {
                            let sym_line = match line_type {
                                LineType::Straight => {
                                    SymbolicLine::Straight(first_point_ent, curr_ent)
                                }
                                LineType::Ray => SymbolicLine::Ray(first_point_ent, curr_ent),
                                LineType::Segment => {
                                    SymbolicLine::Segment(first_point_ent, curr_ent)
                                }
                            };
