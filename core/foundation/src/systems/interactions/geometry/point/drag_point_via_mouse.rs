use specs::prelude::*;
use geopad_core_lib::{math::*, events::*, utilities::*, resources::*, components::{symbolics::*, screen_shapes::*}};
use crate::{utilities::hitting_object, resources::*, events::*};
        ...
        match event {
          MouseEvent::DragBegin(start_position) => {
            if !input_state.keyboard.is_shift_activated() {
              if let Some(entity) = hitting_object(*start_position, &spatial_entity_map, &scrn_points, &scrn_lines, &scrn_circles, SELECT_DIST_THRES) {
          ...
          MouseEvent::DragMove(_, curr_position) => {
            match self.dragging_point {
              Some((ent, _)) => if let Some(old_sym_point) = sym_points.get(ent) {
                if let Some(new_sym_point) = get_update(*old_sym_point, *curr_position, &viewport, &scrn_lines, &scrn_circles) {
                  command_event_channel.single_write(CommandEvent::Update(UpdateEvent::UpdatePoint(ent, *old_sym_point, new_sym_point)));
                }
              },
              None => (),
            }
          ...
          MouseEvent::DragEnd(curr_position) => {
            match self.dragging_point {
              Some((ent, old_sym_point)) => {
                if let Some(new_sym_point) = get_update(old_sym_point, *curr_position, &viewport, &scrn_lines, &scrn_circles) {
                  command_event_channel.single_write(CommandEvent::Update(UpdateEvent::UpdatePointEnd(ent, old_sym_point, new_sym_point)));
                }
              },
              None => (),
            }
