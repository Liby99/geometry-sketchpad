...
use specs::prelude::*;
use geopad_core_lib::{math::*, utilities::*, events::*, resources::*, components::{markers::*, screen_shapes::*}};
      ...
      for event in mouse_event_channel.read(reader_id) {
        match event {
          MouseEvent::MouseDown(mouse_pos) => {
            ...
            if let Some(entity) = hitting_object(*mouse_pos, &*spatial_entity_map, &scrn_points, &scrn_lines, &scrn_circles, SELECT_DIST_THRES) {
          ...
          MouseEvent::DragBegin(start_position) => {
            ...
            if hitting_object(*start_position, &*spatial_entity_map, &scrn_points, &scrn_lines, &scrn_circles, SELECT_DIST_THRES).is_none() {
          ...
          MouseEvent::DragMove(_, curr_position) => {
            ...
            if let Some(start_position) = self.drag_start_position {
              ...
              let rect = AABB::two_points(start_position.into(), (*curr_position).into());
              select_rectangle.set(rect);

              // Select all the elements intersecting with AABB
              let mut new_entities = get_entities_in_aabb(rect, &*spatial_entity_map, &scrn_points, &scrn_lines, &scrn_circles);
